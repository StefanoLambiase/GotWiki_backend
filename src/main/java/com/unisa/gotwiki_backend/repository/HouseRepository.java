...
public interface HouseRepository extends Neo4jRepository<HouseEntity, Long> {
    ...
    @Query("MATCH (h:House), (c:Character{house : h.name}), ((c)-[k:KILLED]->())\n" +
            ...
            "RETURN houseEntity.region AS houseRegion ,houseEntity.alive AS status, houseEntity.words AS houseWords,houseEntity.seat AS houseSeat,houseEntity.name AS houseName, houseEntity.religion AS houseReligion, houseEntity.lord AS houseLord, houseEntity.coa AS houseCoa, killCount\n" +
    ...
    @Query("MATCH (h:House), (c:Character{house:h.name}), (()-[k:KILLED]->(c))\n" +
            ...
            "RETURN houseEntity.region AS houseRegion ,houseEntity.alive AS status, houseEntity.words AS houseWords,houseEntity.seat AS houseSeat,houseEntity.name AS houseName, houseEntity.religion AS houseReligion, houseEntity.lord AS houseLord, houseEntity.coa AS houseCoa, deathCount\n" +
